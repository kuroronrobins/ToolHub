...
class _WindowsCredentialStore:
    ...
    def read(self, target_name: str) -> str | None:
        ...
        ok = self._cred_read(target_name, CRED_TYPE_GENERIC, 0, ctypes.byref(credential))
        if not ok:
            code = ctypes.get_last_error()
            if code == 1168:
                return None
        ...
        try:
            size = int(credential.contents.CredentialBlobSize or 0)
            if size <= 0:
                return None
    ...
    def write(self, target_name: str, key: str) -> None:
        ...
        credential.Comment = "AgendaSnap API key"
        credential.CredentialBlobSize = len(blob)
        credential.CredentialBlob = ctypes.cast(blob_buf, ctypes.POINTER(ctypes.c_ubyte))
        credential.Persist = CRED_PERSIST_LOCAL_MACHINE
        credential.AttributeCount = 0
        credential.Attributes = None
        credential.TargetAlias = None
        credential.UserName = "AgendaSnap"
        ok = self._cred_write(ctypes.byref(credential), 0)
        if not ok:
