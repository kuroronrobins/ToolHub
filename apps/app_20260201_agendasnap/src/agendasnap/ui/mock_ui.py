...
from agendasnap.launcher import entry_command
...
class StreamController:
    ...
    def start(self) -> Path:
        with self._lock:
            ...
            session_root = resolve_session_root(audio_cfg.get("session_root", "sessions"))
...
def main(page: ft.Page) -> None:
    ...
    def _open_caption_window() -> None:
        if controller.session_dir is None:
            cfg = _load_config_yaml(DEFAULT_CONFIG_PATH)
            session_root = resolve_session_root(cfg.get("audio", {}).get("session_root", "sessions"))
            launch_args = ["--session-root", str(session_root)]
        else:
            launch_args = ["--session-dir", str(controller.session_dir)]
        subprocess.Popen(
            entry_command("--window", "caption", "--config", str(DEFAULT_CONFIG_PATH), *launch_args),
            close_fds=True,
        )
    ...
    def _open_minutes_window() -> None:
        if controller.session_dir is None:
            ...
            return
        subprocess.Popen(
            entry_command("--window", "minutes", "--session-dir", str(controller.session_dir)),
            close_fds=True,
        )

    def _open_settings_window(tab: str | None = None) -> None:
        args = entry_command("--window", "settings", "--config", str(DEFAULT_CONFIG_PATH))
        if tab:
            args.extend(["--tab", tab])
        if running["value"]:
