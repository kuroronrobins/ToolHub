...
# - vars:, settings:, selectors:, start:, when <cond>:, whens:, finally:
...
def _parse_list_literal(value: str):
    try:
    ...
    if not inner:
        return []
    return [p.strip().strip('"').strip("'") for p in _split_top_level_commas(inner)]
...
def parse(flow_text: str) -> dict:
    ...
    def parse_vars_block(base_indent: int):
        ...
        while i < len(lines):
            ...
            if m:
                ...
                ast['vars'][k] = v if not v.startswith('[') else _parse_list_literal(v)
    ...
    def parse_selectors_block(base_indent: int):
        ...
        while i < len(lines):
            ...
            while i < len(lines):
                cont_raw = lines[i].strip()
                if not cont_raw or cont_raw.startswith('#'):
                    break
                if indent_of(lines[i]) <= base_indent or not cont_raw.startswith('|'):
                    break
                rhs = f"{rhs} {cont_raw}"
                i += 1
            ast['selectors'][name] = parse_selector_value(rhs)

    def parse_settings_block(base_indent: int):
        nonlocal i
        while i < len(lines):
            if not lines[i] or lines[i].strip().startswith('#'):
                i += 1
            ...
            if v.startswith("[") and v.endswith("]"):
                ...
                if inner:
                    for part in inner.split(","):
                        ...
                        if token:
                            items.append(token)
                ast["settings"][k] = items
            else:
                ast["settings"][k] = v.strip().strip('"').strip("'")
