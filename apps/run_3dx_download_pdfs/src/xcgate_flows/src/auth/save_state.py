...
def _default_runtime_root() -> Path:
    ...
    if base:
        return Path(base) / "XCgate_AutoUpload"
    return Path.home() / ".local" / "share" / "XCgate_AutoUpload"
...
def main():
    ...
    ap.add_argument("--storage-state", default=str(_default_runtime_root() / ".auth" / "mega_state.json"))
