...
def create_app_env(context: StudioContext, requirements_path: Path, rebuild: bool = False) -> AppEnvBuildResult:
...
def run_command(command: list[str], cwd: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run(command, cwd=str(cwd), text=True, encoding="utf-8", errors="replace", stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
...
def build_report(context: StudioContext, app_env_path: Path, python_path: Path | None, source: str, notes: list[str], error: str) -> str:
    status = "FAIL" if error else "PASS"
    lines = [
        "# App Env Build Report",
        "",
