...
def run_import(args: argparse.Namespace, repo_root: Path) -> int:
    ...
    import_plan = {
        ...
        "sensitive_excluded_directories": inventory.sensitive_excluded_directories[:100],
        "sensitive_excluded_files": inventory.sensitive_excluded_files[:100],
        "toolhubignore_patterns": inventory.toolhubignore_patterns,
        "suggested_toolhubignore_report": str(context.output_dir / "suggested_toolhubignore.md"),
    ...
    if plan.mode == "shared-env":
        ...
        try:
            ...
            raise
        mark_registration_breakdown(timings, registration_breakdown)
        with timings.phase("shared_runtime_registry"):
            update_shared_runtime_registry(context, shared_runtime_result.env_id, shared_runtime_result.env_path, shared_runtime_result.lock_path)
